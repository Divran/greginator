...
local enchantments = {
	-- GTCE no longer has enchantments for tools
    --[[
    vinteum = "Fortune I",
...
local recommended = {
	["Compressed wrought iron"] = "Good early-game material.<small>Is a direct upgrade to wrought iron, if you go into the optional pneumaticraft side-quest path.</small>",
	["Thaumium"] = "Excellent early-game material if you do some thaumcraft research.",
	["Cobalt"] = "Good early-game material<br><small>Has low durability, but very high mining speed.</small>",
	["Ultimet"] = "All around excellent mid-game material.",
	["Wrought iron"] = "Good early-game material<br><small>If you ever make iron tools - don't. Upgrade it to wrought iron first!</small>",
	["Blue steel"] = "Great mid-game material<br><small>This material can be made as soon as you have your first electric blast furnace.",
	["Damascus steel"] = "Best early-game material.<br><small>Available as loot in villages, with high durability, fast mining speed.</small>",
	["HSS-E"] = "Best non-fusion material in the game.<br><small>Is a direct upgrade to tungsten steel tools.</small>"
}

local function parseLine(line)
    if not string.find(line,"IngotMaterial") then return end
    local name, mining_tier, speed, durability = string.match(line,"^.*IngotMaterial%(%d+,%s\"([%w_]+)\",.-,.-,%s(%d+),.*,%s([%d%.]+)[Ff],%s*[%d%.]+[Ff],%s(%d+).*$")
    ...
    tools[#tools+1] = {
        ...
        enchant = enchant,
        age = 0,
        recommended = recommended[name] or nil
    }
end
...
local str = [[
	CUSTOM IT3 MATERIALS

	val materialCompressedWroughtIron = MaterialRegistry.createIngotMaterial(700, "compressed_wrought_iron", 0xC8B4B4, "dull", 2, [<material:iron> * 1, <material:carbon> * 25], 9.5F, 7.0F, 460);
	val materialCompressedIron = MaterialRegistry.createIngotMaterial(701, "compressed_iron", 0xC8D4D4, "dull", 2, [<material:iron> * 1], 6.5F, 7.0F, 435);
	val materialEnderium = MaterialRegistry.createIngotMaterial(702, "enderium", 0x2E574F, "shiny", 3, null, 8.0F, 7.0F, 256, 3000);
	val materialSignalum = MaterialRegistry.createIngotMaterial(703, "signalum", 0xFFAA33, "shiny", 2, null, 5.0F, 9.0F, 128, 1000);
	val materialThaumium = MaterialRegistry.createIngotMaterial(706, "thaumium", 0x7A007A, "shiny", 3, null, 10.0F, 8.0F, 315);
